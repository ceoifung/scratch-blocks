...
"""))
  ...
  def format_js(self, code):
    ...
    keys_pipe_separated = reduce(
        lambda accum, key: accum + "|" + key, key_whitelist)
    begin_brace = re.compile(r"\{(?!%s)" % (keys_pipe_separated,))
    end_brace = re.compile(r"\}")

    def end_replacement(match):
      try:
        maybe_key = match.string[match.string[:match.start()].rindex(
            "{") + 1:match.start()]
      except ValueError:
        return "}}"
...
class Gen_compressed(threading.Thread):
  ...
  def gen_core(self, vertical):
    ...
    if not os.path.exists('goog'):
      shutil.copytree(
          'node_modules\\google-closure-library\\closure\\goog', 'goog')
  ...
  def do_compile_jar(self, params, target_filename):
    dash_params = ["--compilation_level SIMPLE", "--language_in ECMASCRIPT_2017",
                   "--language_out ECMASCRIPT5", "--define='goog.DEBUG=false'", "--rewrite_polyfills=false"]
    ...
    if sys.platform == "darwin":
      proc = subprocess.Popen(
          " ".join(args), stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    else:
      proc = subprocess.Popen(
          " ".join(args), stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    (stdout, stderr) = proc.communicate()

    # print stdout
    print stderr
    # Build the JSON response.
    filesizes = [os.path.getsize(value)
                 for (arg, value) in params if arg == "js_file"]
    return dict(
        compiledCode=stdout,
        statistics=dict(
  ...
  def run(self):
    ...
    try:
      ...
      cmd = [
          ...
          "--source_synonym_file", os.path.join("msg",
                                                "json", "synonyms.json"),
          "--source_constants_file", os.path.join(
...
if __name__ == "__main__":
  ...
  Gen_compressed(search_paths_vertical,
                 search_paths_horizontal, closure_env).start()
